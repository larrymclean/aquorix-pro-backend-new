 ...
 *  - 2026-02-26 — v8.3.2
 *     - Webhook fix - insert stripe_checkout_session_id
 * ============================================================================
 */
...
async function bestEffortInsertPaymentEvent(pool, event) {
  ...
  const obj = event && event.data && event.data.object ? event.data.object : null;

  // Extract IDs defensively across event types:
  // - checkout.session.* => obj.id starts with "cs_"
  // - payment_intent.*   => obj.id starts with "pi_"
  // - charge.*           => obj.payment_intent often exists
  let stripeCheckoutSessionId = null;
  let stripePaymentIntentId = null;

  if (obj) {
    if (obj.id && typeof obj.id === "string") {
      if (obj.id.startsWith("cs_")) stripeCheckoutSessionId = String(obj.id);
      if (obj.id.startsWith("pi_")) stripePaymentIntentId = String(obj.id);
    }

    // Fallbacks
    if (!stripePaymentIntentId && obj.payment_intent) {
      stripePaymentIntentId = String(obj.payment_intent);
    }
    // Some event objects may include a checkout_session pointer
    if (!stripeCheckoutSessionId && obj.checkout_session) {
      stripeCheckoutSessionId = String(obj.checkout_session);
    }
  }
  ...
  await pool.query(
    ...
    VALUES ($1, $2, $3::jsonb, NOW(), 'received', $4, $5)
    ON CONFLICT (event_id) DO NOTHING
    `,
    [eventId, eventType, payload, stripeCheckoutSessionId, stripePaymentIntentId]
  );
}
